...
def get_score_references(table_access):
    ...
    path_2 = "fda_risk.transformed.locations_ref"
    ...
    return base_deductions_dict, multiplier_dict, locations_df,columns_dict
...
def rollup_scores(df,locations_df):
    ...
    lat_lon = df.groupby(["company_name", "company_address"])[["Lat", "Lon"]].mean().reset_index()
    address_ref_df = address_ref_df.merge(lat_lon, on=["company_name", "company_address"], how="left")
    final_df = address_ref_df.drop(columns=["_sorted_g"])
    # final_df = address_ref_df.merge(locations_df, how="inner", on="letter_id")
    return final_df

def get_lat_lon(row, google_api_key):
    gmaps = googlemaps.Client(key=google_api_key)
    result = gmaps.geocode(row["company_address"])
    if result:
        lat = result[0]["geometry"]["location"]["lat"]
        lon = result[0]["geometry"]["location"]["lng"]
        return [lat, lon]
    print(f"Not found: {row['company_address']}")
    return [None, None]
...
def insert_table(df, path, table_access):
    connection = sql.connect(
        server_hostname="dbc-9ec227be-73c5.cloud.databricks.com",
        http_path="/sql/1.0/warehouses/823db32bd2b9a836",
        access_token=table_access
    ...
    placeholders = ", ".join(["?"] * len(df.columns))  # ← changed %s to ?
    cursor.executemany(f"INSERT INTO {path} VALUES ({placeholders})", rows)
    cursor.close()
    connection.close()
...
def get_secret():
    secret_list = ["fda_fei_api", "anthropic","databricks-fda-project","google_maps"]
    for item in secret_list:
        ...
        if item == "google_maps":
            google_api_key = secret_dict['api_key']

    return headers, ai_key, table_access, google_api_key

#This output is used to check against existing outputs to determine if 
# the FDA has added new letters. 
def get_all_crl_records():
    ...
    lambda x: x[0].replace(" ", "") if isinstance(x, list) and len(x) > 0 else str(x).replace(" ", ""))
    df['letter_date'] = pd.to_datetime(df['letter_date'], format='mixed').dt.strftime("%Y%m%d").astype(str)
    df['letter_id'] = df['application_number'] + df['letter_date']
    df = df.dropna(subset=['company_name', 'company_address', 'text'])
    complete_letter_id_list = df['letter_id'].tolist()
    return df, complete_letter_id_list
...
def get_params_by_round(row):
    ...
    return Params   
def replace_table(df, path, table_access):
    connection = sql.connect(
        server_hostname="dbc-9ec227be-73c5.cloud.databricks.com",
        http_path="/sql/1.0/warehouses/823db32bd2b9a836",
        access_token=table_access
    ...
    cursor = connection.cursor()
    cursor.execute(f"TRUNCATE TABLE {path}")
    rows = [tuple(row) for _, row in df.iterrows()]
